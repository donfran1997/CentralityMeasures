...
PQ newPQ() {
    ...
    //creates a new empty priority queue and make sure
    //it is valid
...
void addPQ(PQ q, ItemPQ item) {
    ...
    while (curr != NULL) {
        ...
        if (curr->item.key == item.key) {
            ...
            //make sure to exit the function if key does exist
            return;
        }
        curr = curr->next;
    }

    //If an item with 'key' doesn't exists intert it into
    //linked list
    ...
    //if queue isn't empty then just append the value to the back
    //this solves the FIFO issue
    } else {
        q->last->next = new;
        new->next = NULL;
        q->last = new;
...
void  showPQ(PQ q) {
    ...
    //print out the values and keys of the priority queue
    while(curr != NULL){
